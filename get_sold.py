...
def get_sold():
    ...
    try:
        ...
        for val in api_response:
            api_instance = meli.RestClientApi(api_client)
            try:
                resource = '/highlights/MCO/category/'+val['id'] # A resource example like items, search, category, etc.
                access_token = 'APP_USR-5052533537567494-081211-73dc630197472657716ff0d935be6123-1167652302' # Your access token.
                api_sold_res = api_instance.resource_get(resource, access_token)
                list_sold.append(api_sold_res['content'])
            except:
                pprint('404 not found')

        pprint(list_sold)
        columns = ['id','position','type']

        
        with open('most_sold.csv', 'a') as csvfile:
